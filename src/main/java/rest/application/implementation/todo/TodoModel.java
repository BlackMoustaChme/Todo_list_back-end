package rest.application.implementation.todo;
...
public class TodoModel implements ITodo {
    ...
    public ArrayList<Todo> getSharedTodosReadable(Integer id) {
        ...
        try {
            ArrayList<Todo> todos = shareRepository.getSharedTodosReadable(id);
            for (Todo todo: todos) {
                try {
    ...
    public ArrayList<Todo> getSharedTodosRedactable(Integer id) {
        ArrayList<Todo> sharedTodos = new ArrayList<>();
        try {
            ArrayList<Todo> todos = shareRepository.getSharedTodosRedactable(id);
            for (Todo todo: todos) {
                try {
                    sharedTodos.add(todoRepository.getTodo(todo.getId()));
                } catch (Exception e) {
    ...
    public void getNumberOfCheckedTodos(Integer id, String clientId) {
        executor.execute(()-> {
            try {
                ...
                timer.setId(clientId);
                timer.updateMessage(clientId, "You have completed " + number + " task/s. Don't forget to delete them");
            } catch (Exception e) {
                Logger.getLogger(TodoModel.class.getName()).log(Level.INFO, null, e);
            }
        });
